...
export const proxyProviderLatencyTestAPI = (
  providerName: string,
  proxyName: string,
  url: string,
  timeout: number,
...
export const proxyLatencyTestAPI = (
  ...
  if (provider !== '') {
    return proxyProviderLatencyTestAPI(provider, proxyName, url, timeout)
  }
...
export const frontendReleaseAPI = async (
  ...
  const { tag_name, body } = await githubAPI
    .get(`repos/Plutonium141/metacubexd-classic/releases/latest`)
